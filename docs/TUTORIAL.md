...
- `./.env`: arquivo central para apontar o endpoint do S3/MinIO (`S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS`, `S3_SECRET`, `S3_SECURE`) e as credenciais do banco (`DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASSWORD`). Também define o `LOG_LEVEL`.
...
1. **Configuração**: `load_config` lê `.env` e injeta credenciais. CLI e serviços dependem do objeto `Config`.
